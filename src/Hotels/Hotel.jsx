...
const Hotel = () => {
    ...
    let primaryURL = `http://localhost:8080/hotels?_page=1&_limit=12`;
    let url = `http://localhost:8080/hotels?_page=${page}&_limit=12`;
    const fetchApiData = async (sortOrder, page, url, search) => {
        ...
        if(sortOrder === "default") {
            ...
            setIsLoading(false);
            url = `http://localhost:8080/hotels?q=${search}`
        }
        console.log(url);
        ...
        try{
            ...
            let pageNumbersTemp = Array.from({ length: totalPagesFinal }, (_, index) => index + 1);
    ...
    return /*isLoading ? <h1 className={styles.loading}>Page Loading, Please Wait a Moment !</h1> : */(<div>
        <div className={styles.filters_div}>
            <div className={styles.search}>
                <input onChange={(e) => (setSearch(e.target.value))} type="text" name="" id="" placeholder='Search'/>
            </div>
        ...
        {isLoading ? <h1 className={styles.no_items}>Page Loading, Please Wait a Moment !</h1> : ""};
        {currItems === 0 && isLoading === false ? <h1 className={styles.no_items}>No Results Found !</h1>: ""};
        {isError ? <h1 className={styles.no_items}>Oops !!! Error Occured While Loading the Page.</h1> : ""}
        <div className={styles.pagination}>
            {pageNumbers.map((num, index) => {
                return <button onClick={() => setPage(num)} key={index}>{num}</button>
            })}
        </div>
    </div>)
}
